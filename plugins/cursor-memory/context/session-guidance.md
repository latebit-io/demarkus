...
Your soul is a local, versioned demarkus store reached through the `demarkus-memory` MCP tools. It is not the host's built-in memory feature; "recall" and "record" in this guidance always mean the soul. Treat it like AGENTS.md or Cursor rules: consult before starting, record as you go, unprompted.
