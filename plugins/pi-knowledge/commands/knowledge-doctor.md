...
- **blank** → every joined system, all worlds. Read the registry for the joined systems:
  ...
  ```
  Each line is a system's MCP server slug; reach it through the destination server's `mark_*` tools. If the file is missing/empty, tell the user no knowledge system is joined (suggest `/knowledge-join <broker-url>`) and stop.
...
2. **Inventory.** `mark_list mark://<world>/` (recurse into subdirectories) for the full set of documents that actually exist in the world.
   ...
   Any failure, truncation, partial-result marker, or unavailable subtree makes the inventory incomplete. Preserve returned entries, list missing scope, and suppress definitive orphan conclusions.
