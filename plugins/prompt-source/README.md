...
A brand renders a base target under another plugin name into `plugins/brands/<name>/`. Sources: `brands` list in `manifest.json`, or `--brands <file>` (`{"brands": [...]}`, same fields) for a downstream repository rendering against an unmodified checkout. No brands shipped here. Guide: [plugins/brands/README.md](../brands/README.md).
...
Every byte here is paid in tokens: session guidance on every session, command and skill bodies on every invocation, and MCP tool descriptions (in `client/cmd/demarkus-mcp` and `tools/internal/broker/gateway/mcp_tools_list.go`) on every request in hosts that eager-load schemas. Rules for any edit:
