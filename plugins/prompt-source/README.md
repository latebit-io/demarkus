...
The runtime prompt files shipped by the eight agent plugins (claude-code, claude-code-knowledge, opencode-memory, opencode-knowledge, pi-memory, pi-knowledge, cursor-memory, cursor-knowledge) are generated from the templates here. `memory/` and `knowledge/` hold the `commands/`, `context/`, and `skills/` template trees; `manifest.json` maps each template to its per-surface output targets.
...
Surface partial: `partials/<name>.tmpl`. Include with `{{template "<name>.tmpl" .}}`.
...
A brand renders a Claude Code base target under another plugin name into `plugins/brands/<name>/`. Brands come from a `brands` list in `manifest.json` or from a separate file passed as `--brands <file>` (`{"brands": [...]}`, same fields), so a downstream repository can render its brands against an unmodified checkout. This repository ships no brands. See [plugins/brands/README.md](../brands/README.md) for the rebranding guide.
