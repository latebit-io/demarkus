...
The canonical layout for a project's memory, lifted from the structure the demarkus-soul knowledge base evolved in practice. Every project lives under `/<project>/` — the slug is the basename of the project directory (the session cwd), lowercased, spaces → hyphens.
