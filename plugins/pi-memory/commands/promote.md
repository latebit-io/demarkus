...
1. **Detection gate.** Run `${DEMARKUS_SCRIPTS}/detect-promote.sh` via Bash. It lists every destination, one per line:
   ...
   - `NONE` → no destination is configured, so promotion is dormant. Offer the two ways to light it up, and act on the user's choice:
     ...
     - a plain remote server: register it now — confirm its MCP server slug (from the pi-mcp-adapter server list (`/mcp` or `pi config`)) and the write path/prefix, then run `${DEMARKUS_SCRIPTS}/promote-target.sh add <slug> <path> [label]` and continue.
