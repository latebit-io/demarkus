...
1. **Detection gate.** Run `${DEMARKUS_SCRIPTS}/detect-promote.sh` via Bash. `NONE` → there is nowhere to promote to, so a scan is pointless; say so and stop. Otherwise note the destinations (`knowledge <slug>` and/or `target <slug> <path>`) — candidates can be promoted to any of them via `/promote`.
