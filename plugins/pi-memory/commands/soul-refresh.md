...
3. **Check each against the live copy.** Through the destination server's `mark_*` tools, call `mark_fetch` and `mark_versions` to read the current version of `mark://<dest>/<path>` — call it `<M>`.
