...
- **blank** → every joined system, all worlds. Read the registry for the joined systems:
  ...
  ```
  Each line is a system's MCP server slug; its tools are `mcp__<slug>__mark_*`. If the file is missing/empty, tell the user no knowledge system is joined (suggest `/knowledge-join <broker-url>`) and stop.
...
- **Metadata lost across versions** *(legacy servers)* — an untagged doc that **used to** carry tags. Brokers before the APPEND metadata merge passed only `{agent: …}` on append, so every `mark_append` dropped the doc's `tags`, `importance`, `title`, and `type`. Once every world runs the merge, this check only ever finds pre-existing damage; retire it when the catalog is clean. On a knowledge system this also silently voids the **policy tag axes** the doc satisfied at publish time, so a compliant doc becomes noncompliant with no write ever being rejected. Run this only on docs the untagged or policy-axis check already flagged, and only when they have more than one version:
  1. `mark_versions` on the doc (a version fetch also reports `current-version`).
  2. Fetch versions newest-first below the current one — `mark_fetch mark://<world>/<path>/v<N>` — and stop at the first whose metadata carries `tags` **and satisfies the policy's required axes and fields**; a version that was itself noncompliant is not a repair, so keep walking and say so if none qualifies. **Cap at 10 fetches per doc**, then try `v1` as a last probe and give up; disclose any doc you gave up on.
  3. Report the recovered `tags`/`importance`/`type`/`title` verbatim and the version they came from, as a **repair candidate**.

  If `mark_versions` or any versioned `mark_fetch` errors, say so and mark that doc **inconclusive**. Never infer metadata a call failed to return.

  Keep it distinct from **Untagged docs**: a doc untagged since `v1` was never tagged, a curation exercise. A doc that *lost* tags is a candidate, not a verdict: a later `mark_publish` may have dropped them deliberately. Say which, if either, the evidence supports (appends between the two versions point at legacy APPEND damage; a publish points at intent) and get the user's confirmation before restoring.

  Fix, as a separate explicit write step since this command never writes: `mark_fetch` the doc with **`force=true`** (a plain fetch returns an outline for bodies of 8KB or more, and publishing an outline destroys the document), then `mark_publish` that body with the **current version's whole metadata map, the recovered fields layered over it** (`mark_publish` replaces the map, so anything you leave out is deleted, opaque keys included), re-checked against the policy's current required tag axes **and** `require_fields`, and `expected_version` set to the current version. Carry `retention` only if the user asks for it. Don't re-publish the old body; the appended content is real. Publish with `on_conflict: "fail"`: the default `merge` answers a raced write with `status: merge-candidate` and publishes nothing, and a metadata repair has no business merging a stale body. If the fetch or the publish fails, report the exact error, stop, and say the doc is unchanged; a half-applied repair is worse than none. A conflict means the doc moved while you worked, so re-run the check against the new current version rather than reporting either a repair or a clean result.
...
- mark://<world>/<doc>.md — tagged at v3 (category:ops + 6 tags, importance 0.7), none at v7; re-publish the current body with the recovered metadata
