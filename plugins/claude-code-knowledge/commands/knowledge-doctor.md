...
- **blank** → every joined system, all worlds. Read the registry for the joined systems:
  ...
  ```
  Each line is a system's MCP server slug; reach it through `mcp__<slug>__mark_*`. If the file is missing/empty, tell the user no knowledge system is joined (suggest `/knowledge-join <broker-url>`) and stop.
