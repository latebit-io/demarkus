...
- **Policy-axis noncompliance** *(knowledge-only — no soul equivalent)* — fetch `mark://root/.well-known/demarkus/policy.md` once and read the **required tag axes** (e.g. `category:`). Then for each fetched doc, flag any that doesn't satisfy every required `axis:value`. These docs were likely published before the axis was required, or via a `warn`-severity gate. Report with the missing axis and the policy's strictness (`warn`/`block`/`ask`).
