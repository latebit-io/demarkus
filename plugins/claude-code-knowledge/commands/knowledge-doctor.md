...
3. **Cross-world references are existence-checkable** here (they aren't in a single-world soul audit): `mark_worlds` + complete per-world inventories can resolve `mark://<other-world>/<path>`. This un-defers the bare-`mark://` reference check (see Deep checks).
...
2. **Inventory.** Build a directory queue starting with `mark_list mark://<world>/`. Follow each response's `next-cursor` as `cursor` until `complete: true`; then enqueue every returned subdirectory and repeat until the queue is empty. One root page is neither recursive nor necessarily complete. Track visited directory URLs to prevent loops. Bound inventory to 1,000 list calls, including continuation pages, or 20,000 documents per world.
