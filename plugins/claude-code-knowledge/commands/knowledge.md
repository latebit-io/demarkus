...
Orient in the joined organizational demarkus knowledge system(s). Knowledge-system counterpart to `/memory` (local soul index).
...
- Don't confuse with `/memory` (personal, local soul). `/knowledge` is the shared, broker-fronted system(s): different stores, different audiences.
