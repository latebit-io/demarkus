...
Orient in the joined organizational demarkus knowledge system(s). Knowledge-system counterpart to `/memory` (local memory index).
...
2. **Show each system's entry points.** Per slug in scope, use that slug's own MCP tools through `mcp__<slug>__mark_*`; `mark://root/...` resolves inside the selected broker, so never reuse one system's tool for another system's root. Then:
...
- Don't confuse with `/memory` (personal, local memory). `/knowledge` is the shared, broker-fronted system(s): different stores, different audiences.
