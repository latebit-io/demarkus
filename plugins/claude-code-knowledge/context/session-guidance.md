...
- A plain remote endpoint has no broker `root`; through that endpoint's MCP tools, fetch bare `/.well-known/demarkus/policy.md` and `/.well-known/demarkus/template.md` when present. Its MCP server slug is routing identity, not a `mark://` authority.
