...
7. **Publish with provenance.** `mark_publish` to the destination mark:
   ...
   For an update, force-fetch first and use the correct `expected_version`; preserve its complete metadata map, including opaque keys, while replacing the curated `tags`/`importance` and setting any policy-required fields, but omit `retention` unless the user explicitly requested it for this write. For a new doc use 0. Metadata travels in the `metadata` object, never as a `---` block in the body. Include a provenance line linking back to the soul `mark://`/path. For any external origin, first canonicalize it to a non-sensitive identifier: remove URL userinfo, query parameters, fragments, secrets, tokens, and PII by default, then include the remaining reference only with the user's approval. Stop if it cannot be sanitized safely; never copy a raw external URL into the published body. Set `on_conflict: "fail"` on every publish; on conflict, refetch and return to the human gate with the new destination state rather than overwriting or silently merging.
