...
- When a knowledge system is also joined (the demarkus-knowledge plugin), `/promote <soul-path>` lifts a ready soul doc up to the shared catalog (curate → gate → publish → back-stamp), `/promote-scan` sweeps the soul for promotion candidates, and `/soul-refresh` pulls promoted docs back down as their authoritative copies evolve. Publishing a new ADR also nudges promotion. With no knowledge system joined, all of these stay dormant.
