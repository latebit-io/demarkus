...
A style gate also warns at write time on body-shape problems: a frontmatter fence opening the body, a missing `# H1`, em dashes, and duplicate headings (headings are `#section` anchors, so duplicates break inbound links). Fix the body rather than ignoring the warning; `DEMARKUS_STYLE_STRICTNESS` adjusts the severity.
