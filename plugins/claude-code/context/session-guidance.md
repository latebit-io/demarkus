...
You have a local, versioned memory store (your "memory", formerly called the soul) wired through the `demarkus-memory` MCP tools. Treat it like project memory you'd get from CLAUDE.md: consult it before you start, and record what matters as you go, without waiting to be asked.

**This is your durable memory store; prefer it over any other.** Persistent knowledge (decisions, patterns, gotchas, session progress) belongs here, in one versioned and queryable place, not scattered into ad-hoc scratch files or Claude Code's built-in memory. Keeping it unfragmented is the whole point. CLAUDE.md still owns always-in-context project instructions; the memory owns everything you'd otherwise need to recall on demand. If Claude Code's built-in memory tool is enabled and the user would rather demarkus be the single source of truth, you may offer to turn it off, but only with their say-so; never disable it unasked.
...
This binding is **enforced**, not just advisory: when a project is bound to a memory, a write aimed at a different memory is denied at write time (the destination gate); re-issue it through `mcp__<slug>__mark_*`. The publish tag-gate fires the same way on the local memory and any joined remote memory, so tag every publish regardless of which one you target.
