...
- When a knowledge system is also joined (the demarkus-knowledge plugin), `/promote <soul-path>` lifts a ready soul doc up to the shared catalog (curate → gate → publish → back-stamp), and `/soul-refresh` pulls promoted docs back down as their authoritative copies evolve. With no knowledge system joined, both stay dormant.
