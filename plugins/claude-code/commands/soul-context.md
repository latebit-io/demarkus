...
3. **Pull recent journal entries.** List `/<project>/journal/` via `mark_list`. Take today's entry plus the two most recent prior days (UTC `YYYY-MM-DD.md`). For each, `mark_fetch` with `force: true` and read in full. If today's file returns `not-found`, just use the most recent two prior days. Surface any list or other fetch failure and mark the corresponding context incomplete.
