...
Resolve which soul this project uses before any `mark_*` call; a project bound with `/soul-join` or `/soul-default` lives on that soul, not the local one. Establish the absolute project directory (`CLAUDE_PROJECT_DIR`); if unavailable, ask the user which project and stop before running anything below. Then run:

```bash
"$HOME/.demarkus/bin/demarkus-plugin" registry memory-default --list --bind "${CLAUDE_PROJECT_DIR}"
```

Read the last line first: trailing `STALE <slug>` → bound soul no longer available; tell the user to restore it (`/soul-init` when the slug is `demarkus-memory`, `/soul-join` for any other slug) or `/soul-default` to rebind, and stop; never fall back to the local soul when that line is present, whatever precedes it. `EMPTY` → no souls joined: local soul, id `demarkus-memory`. `CATALOG` → `<id>\t<tier>\t<host>\t<insecure>\t<current>` rows; the row marked `*` is this project's soul; no marked row means the local soul. Non-zero exit or malformed output → surface the exact error and stop; never fall back silently. Every `mark_*` call below goes through that soul's server (`mcp__<id>__mark_*`; local soul is `mcp__demarkus-memory__mark_*`). If that server's tools are not connected, say so, name the server, and stop.
...
2. **Confirm the project exists in the soul.** `mark_fetch /<project>/index.md` with `force: true`. On `not-found`, the index may just be missing: `mark_list /<project>/`, following `next-cursor` as `cursor` until `complete: true` (at most 20 pages; require a non-empty, unseen cursor per incomplete page). Only a complete result that is `not-found` or has no documents and no subdirectories means the project has no entries yet: say so, suggest `/soul-journal "<entry>"`, stop. Incomplete listing (budget or cursor failure): report partial context, continue with what was listed. Surface unauthorized, transport, server, malformed-response, and list failures; never treat them as absence. Documents or subdirectories present: note the root index is missing and continue.
