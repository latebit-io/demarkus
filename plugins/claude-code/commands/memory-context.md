...
Resolve which memory this project uses before any `mark_*` call; a project bound with `/memory-join` or `/memory-default` lives on that memory, not the local one. Establish the absolute project directory (`CLAUDE_PROJECT_DIR`); if unavailable, ask the user which project and stop before running anything below. Then run:

```bash
"$HOME/.demarkus/bin/demarkus-plugin" registry memory-default --list --bind "${CLAUDE_PROJECT_DIR}"
```

Read the last line first: trailing `STALE <slug>` → bound memory no longer available; tell the user to restore it (`/memory-init` when the slug is `demarkus-memory`, `/memory-join` for any other slug) or `/memory-default` to rebind, and stop; never fall back to the local memory when that line is present, whatever precedes it. `EMPTY` → no memories joined: local memory, id `demarkus-memory`. `CATALOG` → `<id>\t<tier>\t<host>\t<insecure>\t<current>` rows; the row marked `*` is this project's memory; no marked row means the local memory. Non-zero exit or malformed output → surface the exact error and stop; never fall back silently. Every `mark_*` call below goes through that memory's server (`mcp__<id>__mark_*`; local memory is `mcp__demarkus-memory__mark_*`). If that server's tools are not connected, say so, name the server, and stop.
