---
description: Append an entry to today's journal for the current project
argument-hint: <entry text>
---
<!-- markdownlint-disable MD041 -->
<!-- Code generated by tools/plugin-prompts from plugins/prompt-source; DO NOT EDIT. -->

Append the entry to today's journal file for the current project. One file per day per project, appended as the day progresses.

`$ARGUMENTS` blank or whitespace only: ask for the entry text and stop; no write tool.

## Soul

Resolve which soul this project uses before any `mark_*` call; a project bound with `/soul-join` or `/soul-default` lives on that soul, not the local one. First establish the absolute project directory (`CLAUDE_PROJECT_DIR`); if unavailable, ask the user which project and stop before running anything below. Then run:

```bash
"$HOME/.demarkus/bin/demarkus-plugin" registry memory-default --list --bind "${CLAUDE_PROJECT_DIR}"
```

Read the last line first: trailing `STALE <slug>` means the bound soul is no longer available; tell the user to restore it (`/soul-init` when the slug is `demarkus-memory`, `/soul-join` for any other) or `/soul-default` to rebind, and stop; never fall back to the local soul when that line is present, whatever precedes it. Otherwise `EMPTY` means no souls joined: use the local soul, id `demarkus-memory`. `CATALOG` is followed by `<id>\t<tier>\t<host>\t<insecure>\t<current>` rows; the `*` row is this project's soul; no marked row means the local soul. Non-zero exit or malformed output: surface the exact error and stop; never fall back silently. Every `mark_*` call below goes through that soul's server (`mcp__<id>__mark_*`; local soul is `mcp__demarkus-memory__mark_*`). If that server's tools are not connected, say so, name the server, and stop.

## Steps

1. **Resolve and validate the project slug.** Read absolute `CLAUDE_PROJECT_DIR`; candidate = basename, lowercased, spaces to hyphens. Unset: ask the user which project. Before writing, check whether that candidate subtree already exists in the project's soul. If it does and this session has not established it belongs to this exact absolute path, ask the user to confirm or choose a unique slug; equal basenames never imply the same project. Slug syntax before any path: `^[a-z0-9][a-z0-9._-]*$`; reject empty, `.`/`..`, delimiters (`/`, `\`, `:`, `?`, `#`, `%`). Use the confirmed slug consistently for the journal path and root-index link.

2. **Compute the target path.** Today's UTC date as `YYYY-MM-DD`. Target: `/<project>/journal/<YYYY-MM-DD>.md`.

3. **Check existence and capture pre-write project state.** `mark_list /<project>/` with `include_archived: true`, `next-cursor` → `cursor` until `complete: true`, max 100 calls; each incomplete page needs a non-empty unseen cursor. Bound hit before `complete: true` → failure, not absence. Fetch `/index.md` and `/<project>/index.md` with `force: true` and `verbose: true`; retain: subtree exists, exact `- [<Project>](/<project>/)` root link exists, project hub exists, each hub's body, version, metadata map. Only `not-found` = absent. Any other failure (unauthorized, transport, server, malformed, outline-only) → surface, stop before any journal write. Then `mark_fetch` the target path:

   - `not-found`: file does not exist yet. Create with `mark_publish` (`expected_version: 0`, `on_conflict: "fail"`). If another writer creates it first, settle ownership before touching that file: when step 3 found no `/<project>/` subtree, the concurrent creator may be another checkout with the same basename, so unless this session already established `/<project>/` is this checkout's, ask the user to confirm; stop if they do not. Then fetch the new journal with `force: true` and continue exactly as in the `ok` branch below (complete body required, present-entry rule, one retry, metadata enrichment); never overwrite or drop either entry. **Set a `metadata` object so the entry is findable via `mark_lookup`**; you choose these, the server does not infer them:

     - `tags`: comma-separated subjects. Always `journal` and the project slug, plus the concrete topics in the entry (e.g. `auth`, `release`, `bugfix`, `lookup`). Derive from the entry text; don't stop at `journal`.
     - `importance`: float 0–1 for later recall. Routine daily notes ~0.3–0.5; a real decision or milestone higher. 0.4 when unsure.
     - `type`: `Journal`.

     Body:

     ```text
     # <Project> journal: <YYYY-MM-DD>

     <entry text>
     ```

   - `ok`: today's file exists. `mark_append` with:

     ```text

     <entry text>
     ```

     Pass the `expected_version` from the fetch above; that fetch, and every refetch here, must return a complete body (outline-only or malformed: surface the exact error and stop). Before writing, and again after any conflict, apply the present-entry rule: the entry is present only when the complete entry text appears as its own blank-line-delimited block (typically at the end); a substring hit inside another entry, a heading, or a code block is not a match. Present: treat the write as done. Append conflict: refetch with `force: true`, re-apply the rule, retry the append exactly once with the fetched version; second conflict: report the entry as not written and stop. Always prefix the entry body with a leading blank line to separate it from the previous entry.

     `mark_append` carries catalog metadata forward (`retention` excepted), so creation-time `tags`/`importance`/`type` stand. If this entry introduces a materially new subject worth finding later: fetch with `force: true` and `verbose: true`, reject outline-only, re-publish the full current body with the correct `expected_version` and `on_conflict: "fail"`, resending the complete metadata map with extended `tags`; `mark_publish` replaces the map, so a tags-only publish drops `importance`, `title`, and `type`. If that force-fetch or metadata publish fails: report append committed, enrichment incomplete, surface the exact error, stop without repeating `mark_append`. Skip enrichment for routine same-topic appends.

   - Unauthorized, transport, server, malformed-response, or other fetch failure: surface the exact error and stop without writing or claiming success.

4. **Repair hubs.** After any successful journal write, repair both hubs from the saved state. Hub rule: republish only if link missing or metadata off contract; saved body plus link once, saved metadata minus `type` and unrequested `retention`, saved version, `on_conflict: "fail"`.
   - `/<project>/index.md`: `not-found` → create at v0: `# <Project>`, `## Journal` linking today, `tags: index,<slug>`, `importance: 0.8`, no `type`. Present → hub rule; link under `## Journal`, else appended.
   - `/index.md`: `not-found` → create at v0: `# Projects` plus project link, `tags: projects,index`, `importance: 0.9`, no `type`. Present → hub rule; contract: `projects,index` in tags, `importance` 0.9 if unset.

   Conflict: refetch `force: true` (outline or malformed → stop); link present and on contract → done; else keep concurrent content, apply hub rule to the refetch, retry once; second conflict → stop (step 5 reports).

5. **Report operation-specific results.** Check every `mark_publish` and `mark_append` result. Journal write failure: surface the exact result, do not claim it was written. After successful creation/append, report that write as committed and never repeat it because a dependent index or metadata update failed. Report each later failure separately as partial success: journal committed, dependent update incomplete.

Entry: $ARGUMENTS
