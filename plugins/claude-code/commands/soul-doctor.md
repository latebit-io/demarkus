...
2. **Inventory.** `mark_list` the scope, following every `next-cursor` as `cursor` until `complete: true`, then recurse into returned subdirectories for the full set of documents that actually exist.
