...
2. **Find promoted docs.** If `$ARGUMENTS` names a path, use just that one. Otherwise `mark_lookup` the soul with `filter=tag=promoted`, `limit=1000` to list promoted documents (the `/promote` back-stamp adds that tag). If lookup fails, report the exact error, mark the refresh incomplete, and stop. Because lookup has no cursor, exactly 1000 rows means the scan may be truncated: process returned rows only if useful, but mark the refresh incomplete and never claim exhaustive or no-stale results. For each row, `mark_fetch` it with `force: true`, retaining its current version and complete metadata map, and parse both marker lines. Surface each fetch failure, skip that document, and mark the refresh incomplete.
   ...
   `<server-slug>` selects the exact broker or plain endpoint MCP server; `<target>` is a brokered `mark://<world>/<path>` or a plain endpoint's bare `/path`; `<N>` is the destination version this soul copy last tracked. Use the recorded server's tools for every destination call. For a legacy doc with only `promoted:`, ask the user which configured server owns that target; never infer a broker from a `mark://` authority. A legacy plain marker that used `mark://<server-slug>/<path>` is a routing label, not a dialable endpoint: after explicit server selection, use its bare `/path`. Add `promoted-via` and normalize the target on the next confirmed soul write. Other missing or malformed markers are ambiguous — report and skip.

3. **Check each against the live copy.** Through `mcp__<slug>__mark_*` on the recorded server, call `mark_fetch` with `force: true` on the exact brokered target or plain bare path first. Handle its status before calling `mark_versions`:
   ...
   - `<M> == <N>` → in sync. Skip silently unless this is a legacy marker awaiting normalization; offer a confirmed identity-only soul write that adds `promoted-via`/normalizes the target while preserving the body and complete metadata.
