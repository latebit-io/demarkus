...
3. **Check each against the live copy.** Through `mcp__<slug>__mark_*`, call `mark_fetch` with `force: true` on `mark://<dest>/<path>` first. Handle its status before calling `mark_versions`:
