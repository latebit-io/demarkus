...
1. **Detection gate.** Run `"$HOME/.demarkus/bin/demarkus-plugin" registry detect-promote` via Bash. Non-zero exit or malformed output is a registry failure: surface it and stop. A successful command lists every destination, one per line:
   ...
   - `NONE` → no destination is configured, so promotion is dormant. Offer the two ways to light it up, and act on the user's choice:
      ...
      - a plain remote server: register it now — confirm its MCP server slug from `claude mcp list` and write path/prefix, then run `"$HOME/.demarkus/bin/demarkus-plugin" registry promote-target add '<slug>' '<path>' '[label]'` and continue.
   - If more than one destination is listed, ask which to use; otherwise use the single one. Reach any destination through `mcp__<slug>__mark_*`.

2. **Read the source.** Call `mcp__demarkus-memory__mark_fetch` or the configured soul server's `mark_fetch` tool on the soul path with `force: true`. Promotion and marker detection require the complete body, not an outline. Note its current version and complete metadata map — the back-stamp needs both. If `not-found`, say so and stop; surface outline-only, transport, and authorization failures instead of treating them as absence.
...
4. **Run the execution cascade (knowledge side).** Hand the source document — and the chosen destination from step 1 (a brokered system, or a plain `target` with its declared write path) — to the demarkus-knowledge plugin's **`knowledge-promote`** skill, which:
   ...
   - If any cascade lookup, fetch, policy, routing, approval, validation, or publish step fails, or the result lacks a confirmed published URL and version, surface the exact failure and **stop without back-stamping**. A merge candidate or conflict response is not a publish.

5. **Capture the result.** Only from a successful `mark_publish` response, record its exact published document URL and returned version as `mark://<dest>/<path>@v<N>`. Do not reconstruct it from placeholders or continue on malformed output.
...
6. **Back-stamp the soul doc (memory side, this command's job — the knowledge plugin never writes the soul).** Apply provenance to the source so it is not re-promoted and does not drift from the now-authoritative copy. Confirm the mode with the user:
   ...
   Write the back-stamp with `mark_publish` on the soul at the version from step 2 and `on_conflict: "fail"`. Publisher metadata is replaced, not merged: preserve the complete current metadata map, including opaque keys, but omit `retention` unless the user explicitly requested it for this write. Add the `promoted` tag so the doc is discoverable through `mark_lookup filter=tag=promoted`. On version conflict, do not publish the stale body or blindly accept a structural merge: refetch with `force: true`, preserve the concurrent edits and metadata, reapply the chosen stamp mode, and retry against the new version. If intent conflicts, show the user and stop. Reconciliation is **directional**: knowledge is authoritative, the soul refreshes from it, and any local edits re-enter through the gate (step 3) — never a silent two-way merge.
