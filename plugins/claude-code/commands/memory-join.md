...
Connect this Claude Code installation to a remote demarkus soul: a direct-QUIC soul server, or an HTTPS demarkus-memory-broker (hosted personal soul behind OAuth). `/memory-init` manages a local soul; `/knowledge-join` joins an organizational knowledge system.
...
2. Validate, store credentials, add the soul catalog row, bind the absolute current project directory:
