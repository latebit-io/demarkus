...
3. **Check existence and capture pre-write project state.** Before creating anything, call `mark_list /<project>/` with `include_archived: true`, following `next-cursor` until the result reports `complete: true` (an incomplete listing is a failure, not absence), and force-fetch `/index.md`; retain whether the project subtree and exact `- [<Project>](/<project>/)` root-hub link already exist, plus the full root-index body/version when present. Only `not-found` establishes absence. On any other failure (unauthorized, transport, server, malformed, or outline-only response), surface the exact error and stop before any journal write: the slug check in step 1 depends on this state, and writing blind can land the entry under another checkout's project. Then call `mark_fetch` on the target path:
