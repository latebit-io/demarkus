...
{
  bash "${SCRIPTS_DIR}/bootstrap.sh" || echo "[demarkus] bootstrap failed; memory tools may be unavailable this session" >&2
  if [[ -x "${BIN}" ]]; then
...
[[ -x "${BIN}" ]] || exit 0
"${BIN}" guidance --surface memory --guidance-file "${GUIDANCE_FILE}" --format claude || { echo "[demarkus] guidance exited $?; no guidance injected" >&2; exit 0; }
