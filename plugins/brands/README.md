# Rebranding the Claude Code memory and knowledge plugins

Audience: a team that wants the demarkus memory or knowledge plugin for Claude Code under its own plugin name, generated by its own CI and published from its own marketplace.
...
Only Claude Code base targets can be branded. The pi, OpenCode, and Cursor adapters carry their names in TypeScript and install scripts and need a copy step of their own.
...
Fields: `base` is `claude-memory` or `claude-knowledge`; `output` must live under `plugins/brands/` of the demarkus checkout; `plugin_name` is lowercase letters, digits, and hyphens and becomes the `/<plugin_name>:` command prefix; `memory_plugin_name` and `knowledge_plugin_name` name the sibling plugin when you brand both surfaces (one entry per base); `store_noun` and `store_noun_plural` replace the word "soul" in the prompts. Unknown fields are rejected.
...
`write` renders the prompts with your names, copies `hooks/`, `scripts/`, and `.mcp.json` (memory only) from the base plugin, rewrites the name and description in `.claude-plugin/plugin.json` (version and hooks untouched), and writes a README into each `output`. `check` fails on any drift between templates and generated files. Both need the full checkout, since the copied files come from the base plugin.

## Publishing from your own repository

Layout in the repository your users will add as a marketplace (a monorepo works; the marketplace file must sit at the repository root):

```text
.claude-plugin/marketplace.json
claude-plugins/brands.json
claude-plugins/upstream.ref        # one demarkus commit SHA
claude-plugins/acme-brain/         # generated, committed through a PR
claude-plugins/acme-knowledge/
```

Marketplace entry, one per brand; `source` paths are relative to the repository root and `version` must equal the base plugin's version, which the generated `plugin.json` carries:
...
```json
{
  ...
  "plugins": [
    {
      "name": "acme-brain",
      "source": "./claude-plugins/acme-brain",
...
cd ~/src/demarkus && git fetch origin && git checkout "$(cat ~/src/acme/claude-plugins/upstream.ref)"
(cd tools && go run ./plugin-prompts write --brands ~/src/acme/claude-plugins/brands.json)
cd ~/src/acme && git switch -c chore/demarkus-brands
for dir in ~/src/demarkus/plugins/brands/*/; do
  name=$(basename "$dir")
  rm -rf "claude-plugins/$name" && cp -R "$dir" "claude-plugins/$name"
done
# set each marketplace.json version to the one in claude-plugins/<name>/.claude-plugin/plugin.json
git add claude-plugins .claude-plugin/marketplace.json && git commit -m "chore(plugins): regenerate demarkus brands"
gh pr create --fill
```
...
on:
  push:
    branches: [main]
    paths: [claude-plugins/brands.json, claude-plugins/upstream.ref]
  workflow_dispatch:
  ...
  generate:
    ...
    steps:
      ...
      - run: |
          ...
          git -C /tmp/demarkus checkout "$(cat claude-plugins/upstream.ref)"
          brands="$GITHUB_WORKSPACE/claude-plugins/brands.json"
          ...
          for dir in /tmp/demarkus/plugins/brands/*/; do
            name=$(basename "$dir")
            rm -rf "claude-plugins/$name" && cp -R "$dir" "claude-plugins/$name"
            version=$(jq -r .version "claude-plugins/$name/.claude-plugin/plugin.json")
            jq --arg n "$name" --arg v "$version" '(.plugins[] | select(.name == $n) | .version) = $v' \
...
- Users run `/plugin marketplace add <org>/<repo>` and `/plugin install acme-brain@acme`. A private repository needs git access for every user, since the marketplace add clones it.

## Alternative: brands in a fork

A fork can instead add the same entries as a `brands` list in `plugins/prompt-source/manifest.json` after `targets`, run `go run ./plugin-prompts write` with no flag, commit `plugins/brands/`, and publish through a marketplace file in the fork. Staying current is then `git fetch upstream && git merge upstream/main` followed by a regeneration. Prefer the brands file: it needs no merge and keeps the fork's `manifest.json` identical to upstream.
