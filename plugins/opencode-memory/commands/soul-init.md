...
3. **MCP registration is automatic.** The plugin's `config` hook registers the
   `demarkus-memory` server at startup (launched via `demarkus-plugin
   mcp-serve`). Tell the user to restart opencode to connect it. Its tools
   will appear as `demarkus-memory_mark_*`. Do NOT run `registry mcp add`;
   that writes a config OpenCode does not read.
