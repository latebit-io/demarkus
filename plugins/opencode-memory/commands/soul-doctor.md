...
2. **Inventory.** `mark_list` the scope, following every `next-cursor` as `cursor` until `complete: true`, then recurse into returned subdirectories. Track visited paths and directory/cursor pairs; require each incomplete page to return a non-empty, different, unseen cursor. Stop at 500 list calls, 200 directories, or 10,000 documents. Preserve discovered results but mark coverage incomplete and suppress conclusions that require exhaustive inventory when any limit or cursor-progress check fails.
