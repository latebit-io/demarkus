...
2. **Gather the already-promoted set (to exclude).** `mark_lookup` the soul with `filter=tag=promoted`, `limit=1000` (optionally scoped to `/<slug>/`). If the call fails, stop rather than treating the exclusion set as empty. These already live in the catalog — never re-list them as candidates. Their soul docs also carry a `promoted: mark://…@v<N>` marker; `/soul-refresh` keeps them current, not this command.

3. **Find high-signal candidates.** Within scope, surface documents whose *type* makes them likely shared-team knowledge, skipping anything in the already-promoted set. Bound the sweep to 200 listed paths and 40 full-body fetches, prioritizing ADRs, architecture, guidelines, patterns, high importance, and explicit sharing tags. If the inventory exceeds either budget, report that the scan is partial and give the applied limits; never imply exhaustive coverage.
