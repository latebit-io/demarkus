...
Connect this OpenCode installation to a **remote demarkus soul** — a
...
2. **MCP registration is automatic.** The plugin's `config` hook reads the
   souls catalog (`~/.demarkus/souls`) at startup and registers each joined
   soul as an MCP server (launched via `demarkus-plugin mcp-serve --soul
   <slug>`, which injects the token from the 0600 file). Tell the user to
   restart opencode so the new soul's tools load. Do NOT run
   `registry mcp add`; that writes a config OpenCode does not read.
...
- Don't conflate the three soul surfaces:
  ...
  - `/knowledge-join`: organizational, HTTPS broker, OAuth via OpenCode.
