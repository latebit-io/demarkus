...
4. Fetch `/index.md`, add a bullet `- [<Project Name>](/<slug>/)` under the project list, and publish with the correct `expected_version`. If another writer changes it first, use the merge candidate only after checking that both project links remain; otherwise refetch, reapply the link, and retry. Never overwrite a concurrent index update.
