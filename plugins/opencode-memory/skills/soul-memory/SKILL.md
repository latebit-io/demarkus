...
4. Force-fetch `/index.md`. On `not-found`, bootstrap it with a `# Projects` heading and the project link using `expected_version: 0` and `on_conflict: "fail"`. On `ok`, require a complete body, add the link only if absent, preserve every existing link, and publish with the fetched version and `on_conflict: "fail"`. On the first conflict in either path, force-fetch the fresh complete body/version; if the link is now present, stop successfully, otherwise add it once and retry once. If that retry conflicts or any non-`not-found` fetch/publish fails, surface the result and stop. Never publish an outline, accept an unchecked merge candidate, or overwrite a concurrent index update.
