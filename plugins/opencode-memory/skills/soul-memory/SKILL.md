...
4. Force-fetch `/index.md`. On `not-found`, bootstrap it with a `# Projects` heading and the project link using `expected_version: 0`, `on_conflict: "fail"`, and metadata `tags: projects,index` plus `importance: 0.9`; deliberately omit `type` because `index.md` is untyped. On `ok`, require a complete body, retain its complete metadata map, add the link only if absent, preserve every existing link, and publish with the fetched version, that metadata, and `on_conflict: "fail"`; omit `retention` unless the user explicitly requests it for this write. On the first conflict in either path, force-fetch the fresh complete body/version and metadata; if the link is now present, stop successfully, otherwise add it once and retry once while resending the fresh complete metadata map, again omitting unrequested `retention`. If that retry conflicts or any non-`not-found` fetch/publish fails, surface the result and stop. Never publish an outline, accept an unchecked merge candidate, or overwrite a concurrent index update.
