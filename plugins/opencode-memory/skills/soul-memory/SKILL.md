...
4. Force-fetch `/index.md` and require a complete body, add a bullet `- [<Project Name>](/<slug>/)` under the project list only if absent, preserve every existing link, and publish with the correct `expected_version` and `on_conflict: "fail"`. On the first conflict, force-fetch the fresh complete body/version; if the link is now present, stop successfully, otherwise add it once and retry once. If that retry conflicts or any fetch/publish fails, surface the result and stop. Never publish an outline, accept an unchecked merge candidate, or overwrite a concurrent index update.
