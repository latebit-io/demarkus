...
Your soul is a local, versioned demarkus store reached through the `demarkus-memory` MCP tools. It is not the host's built-in memory feature; "recall" and "record" in this guidance always mean the soul. Treat it like repo instructions: consult before starting, record as you go, unprompted.

**Your durable store; prefer it over any other.** Decisions, patterns, gotchas, session progress live here, not in scratch files or a harness's built-in memory. repo instructions owns always-in-context instructions; the soul owns everything recalled on demand. A separate one-time setup offer handles built-in memory integration; never disable anything unasked.
