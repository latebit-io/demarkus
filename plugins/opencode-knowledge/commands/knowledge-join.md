...
6. Once tools are available, fetch and follow:
   ...
   If either fetch fails for any reason other than `not-found`, or if policy mirroring fails, the join is incomplete and must not reach step 7. Report the failure, then roll back the new connection in this order: `opencode mcp logout '<slug>'`, `registry mcp remove '<slug>'`, and `registry knowledge-unregister '<slug>'`. Surface every rollback failure separately and tell the user exactly what remains registered. Allow `/knowledge-join` to run again only after all rollback commands succeed; otherwise require cleanup of the registrations that remain.
