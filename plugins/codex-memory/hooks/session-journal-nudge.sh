#!/usr/bin/env bash
# Codex Stop adapter for the session-end journal nudge. The Stop mechanics are
# harness-specific and stay here: parse the payload, guard against re-blocking,
...
# demarkus-plugin binary. Self-contained — the small JSON field extractor is
# inlined.
#
...
#
# Codex's Stop payload shares Claude's field names (session_id, transcript_path,
# stop_hook_active), so the parser is identical; only the transcript markers
# differ — Codex edits files via `apply_patch` and shell, and surfaces MCP tools
# as mcp__<server>__<tool>. The patterns below cover both harnesses' transcripts,
# so a missed signal degrades to "no nudge", never a false block.
...
soul_write="false"
if grep -qE '"attributionMcpTool":"mark_(publish|append)"|mcp__[^"]*mark_(publish|append)|"name":"mark_(publish|append)"' "${tpath}"; then
  soul_write="true"
fi
changed_files="false"
if grep -qE '"name":"(Edit|Write|NotebookEdit|apply_patch|shell)"|"apply_patch"' "${tpath}"; then
  changed_files="true"
fi

out="$("${BIN}" nudge --event session-end --changed-files="${changed_files}" --soul-write="${soul_write}" --format codex < /dev/null || true)"
if [[ -n "${out}" ]]; then
