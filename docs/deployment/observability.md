...
- **Datadog**: container logs are auto-collected by the Datadog Agent's `kubernetes_state_core` integration; JSON parsing is automatic. Add a `service:demarkus-server` (or `-broker` / `-agent`) tag via the pod annotations Datadog's autodiscovery reads. See [Datadog Agent: Kubernetes log collection](https://docs.datadoghq.com/agent/kubernetes/log/).
...
- **Fluent Bit**: tail input + JSON parser. See [Fluent Bit Kubernetes input](https://docs.fluentbit.io/manual/pipeline/inputs/kubernetes-events).
