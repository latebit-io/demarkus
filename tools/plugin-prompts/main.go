...
func renderAll(root string) ([]artifact, error) {
	...
	if len(spec.Targets) != 6 {
		return nil, fmt.Errorf("manifest: got %d targets, want 6", len(spec.Targets))
	}
...
func validateTarget(target *target) error {
	if target.Name == "" || target.Output == "" {
		return errors.New("manifest: target name and output are required")
	}
...
func addMarker(content []byte) []byte {
	...
	if strings.HasPrefix(text, "---\n") {
		if end := strings.Index(text[4:], "\n---\n"); end >= 0 {
			at := end + 9
			return []byte(text[:at] + generatedMarker + "\n" + text[at:])
		}
	}
	return []byte(generatedMarker + "\n" + text)
}
...
func checkAll(root string, artifacts []artifact) error {
	...
	for i := range artifacts {
		...
		for _, subtree := range []string{"commands", "context", "skills"} {
			dir := filepath.Join(root, artifact.Target.Output, subtree)
