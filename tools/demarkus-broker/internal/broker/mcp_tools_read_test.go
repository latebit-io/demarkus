...
func TestHandleMarkFetchHappyPath(t *testing.T) {
	...
	if call.token == "" {
		t.Error("dispatcher saw empty token; broker must lazy-mint and forward")
	}
}
...
func TestHandleMarkFetchCacheHitReusesToken(t *testing.T) {
	cfg := mcpTestConfig()
	...
	ctx := withAliceClaims(context.Background())
	for range 3 {
		if res, err := g.handleMarkFetch(ctx, callToolReq("mark_fetch", map[string]any{
			"url": "mark://team-a/foo.md",
		})); err != nil || res.IsError {
			t.Fatalf("handleMarkFetch: err=%v isError=%v", err, res.IsError)
		}
	}
	mu.Lock()
	defer mu.Unlock()
	if len(capturedTokens) != 3 {
		t.Fatalf("dispatcher call count = %d, want 3", len(capturedTokens))
	}
	// All three calls should carry the SAME token — the cache
	// hit reuses the previously-minted token without another
	// Issuer round-trip.
	for i := 1; i < 3; i++ {
		if capturedTokens[i] != capturedTokens[0] {
			t.Errorf("dispatch %d token = %q, want %q (cache reuse)", i, capturedTokens[i], capturedTokens[0])
		}
	}
}

func TestDispatchReadRetriesOnFreshMint401(t *testing.T) {
	cfg := mcpTestConfig()
	// Tight retry knobs so the test runs fast. Three attempts,
	// 1ms initial, 2ms cap.
	...
	d := &fakeDispatcher{
		fetchFn: func(_, _, _ string) (fetch.Result, error) {
			...
			if n < 3 {
				// First two attempts: world hasn't seen the
				// freshly-minted token yet (kubelet projected-
				// volume refresh still pending). Returns
				// unauthorized, which is the broker's retry
				// signal.
	...
	if res.IsError {
		t.Fatalf("isError = true after successful retry: %s", toolResultText(t, res))
	}
	if got := atomic.LoadInt32(&attempt); got != 3 {
		t.Errorf("dispatcher attempts = %d, want 3 (two 401 then ok)", got)
	}
	text := toolResultText(t, res)
	if !strings.Contains(text, "status: ok") {
		t.Errorf("retry success result missing status: ok\nfull:\n%s", text)
	}
}

func TestDispatchReadExhaustsRetriesOnPersistent401(t *testing.T) {
	cfg := mcpTestConfig()
	cfg.Server.MCP.FirstMintMaxAttempts = 3
	cfg.Server.MCP.FirstMintInitialBackoff = time.Millisecond
	cfg.Server.MCP.FirstMintMaxBackoff = 2 * time.Millisecond
	...
	if !res.IsError {
		t.Fatal("isError = false after persistent 401, want true")
	}
	if got := atomic.LoadInt32(&attempts); got != 3 {
		t.Errorf("dispatcher attempts = %d, want 3 (max attempts)", got)
	}
	text := toolResultText(t, res)
	if !strings.Contains(text, "propagation lag") {
		t.Errorf("tool error message = %q, want it to name the propagation-lag cause", text)
	}
}

func TestDispatchReadCacheHit401InvalidatesAndReMints(t *testing.T) {
	cfg := mcpTestConfig()
	// Tight retry to keep the test fast even if a backoff fires
	// after the re-mint.
	cfg.Server.MCP.FirstMintMaxAttempts = 3
	cfg.Server.MCP.FirstMintInitialBackoff = time.Millisecond
	cfg.Server.MCP.FirstMintMaxBackoff = 2 * time.Millisecond
	...
	d := &fakeDispatcher{
		fetchFn: func(_, _, token string) (fetch.Result, error) {
			...
			mu.Unlock()
			// Second call returns unauthorized once — the
			// cache-hit token has gone stale. Third call (after
			// re-mint) succeeds.
	...
	if res, err := g.handleMarkFetch(ctx, callToolReq("mark_fetch", map[string]any{"url": "mark://team-a/foo.md"})); err != nil || res.IsError {
		t.Fatalf("first call: err=%v isError=%v", err, res.IsError)
	}
	// Second call: cache hit → 401 → invalidate → re-mint → ok.
	if res, err := g.handleMarkFetch(ctx, callToolReq("mark_fetch", map[string]any{"url": "mark://team-a/foo.md"})); err != nil || res.IsError {
		t.Fatalf("second call: err=%v isError=%v", err, res.IsError)
	}
	mu.Lock()
	defer mu.Unlock()
	if len(seenTokens) != 3 {
		t.Fatalf("dispatcher call count = %d, want 3 (first seed + cache-hit 401 + re-mint)", len(seenTokens))
	}
	// First two calls used the same token (the originally-minted
	// one); third call uses a different token (the re-mint).
	if seenTokens[0] != seenTokens[1] {
		t.Errorf("tokens[0]=%q tokens[1]=%q — cache hit should reuse the original token before invalidation", seenTokens[0], seenTokens[1])
	}
	if seenTokens[2] == seenTokens[0] {
		t.Errorf("tokens[2]=%q matches tokens[0]; re-mint should produce a different value", seenTokens[2])
	}
}
...
func TestHandleMarkFetchUnauthorizedIdentitySurfacesAsToolError(t *testing.T) {
	...
	// Identity whose email domain doesn't match the world's
	// allow.domains (only example.com is allowed in testConfig).
	...
	if !res.IsError {
		t.Fatal("isError = false for unauthorized identity, want true")
	}
	text := toolResultText(t, res)
	if !strings.Contains(text, "not authorized") {
		t.Errorf("tool error = %q, want \"not authorized\" message", text)
	}
}
