...
func cmdMcpServe(args []string) {
	...
	if *memory == "" {
		...
		if written, err = registry.SetLocalMemoryAlias(*name); err != nil {
	...
	if err := syscall.Exec(mcpBin, argv, env); err != nil {
		...
		if *memory == "" {
			// Only while our record is still current: a later mcp-serve may have started.
			if rerr := registry.RestoreLocalMemoryAlias(previous, written); rerr != nil {
