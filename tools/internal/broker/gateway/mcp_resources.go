...
func (g *Gateway) registerResources() {
	...
	worlds := core.ReadableWorlds(g.deps.Worlds)
	for j := range worlds {
		w := &worlds[j]
...
func (g *Gateway) readResource(ctx context.Context, req mcp.ReadResourceRequest) ([]mcp.ResourceContents, error) {
	raw := req.Params.URI
...
func (g *Gateway) admitResourceRead(ctx context.Context, raw string) (context.Context, error) {
	if !g.profile.TenantScoped {
		return ctx, nil
	}
