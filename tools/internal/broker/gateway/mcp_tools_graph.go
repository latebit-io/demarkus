...
func (g *Gateway) crawlFetchFn(ctx context.Context) graphstore.FetchFunc {
	tenant := ""
	if g.profile.TenantScoped {
		...
		if err != nil {
			...
			return func(context.Context, links.Target) (graph.FetchResult, error) {
				return graph.FetchResult{}, core.ErrNotAuthorized
			}
		}
		tenant = w.Name
	}
...
func (g *Gateway) seedGraphStore(ctx context.Context, state *gatewayGraph) {
...
func (g *Gateway) translateSeedURLs(nodes []graphstore.StoredNode, edges []graphstore.StoredEdge) {
...
func (g *Gateway) handleMarkGraphExport(ctx context.Context, _ mcp.CallToolRequest) (*mcp.CallToolResult, error) { //nolint:gocritic // signature required by mcp-go
