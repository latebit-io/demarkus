...
func TestWorldWriteTokenStoreProvisionIsIdempotent(t *testing.T) {
	...
	store := newWorldWriteTokenStore(cfg, storage.NewK8sSecretStore(k8s))
	...
	brokerSecret, err := k8s.CoreV1().Secrets(cfg.Server.BrokerNamespace).Get(context.Background(), core.WorldWriteTokenRef(cfg, "team-a").Name, metav1.GetOptions{})
	if err != nil {
	...
	body := string(worldSecret.Data[core.TokensSecretKey])
...
func TestWorldWriteTokenStoreProvisionRecoversAcrossStores(t *testing.T) {
	...
	podA := newWorldWriteTokenStore(cfg, storage.NewK8sSecretStore(k8s))
	first, err := podA.Provision(context.Background(), "team-a")
	if err != nil {
		t.Fatalf("podA.Provision: %v", err)
	}

	podB := newWorldWriteTokenStore(cfg, storage.NewK8sSecretStore(k8s))
...
func TestWorldWriteTokenStoreRewritesStaleWorldEntry(t *testing.T) {
	...
	k8s := fake.NewSimpleClientset(&corev1.Secret{
		...
		Data:       map[string][]byte{core.TokensSecretKey: staleBytes},
	})
	...
	parsed, err := token.ParseBytes(worldSecret.Data[core.TokensSecretKey])
	if err != nil {
...
func TestWorldWriteTokenStoreProvisionUnknownWorld(t *testing.T) {
	...
	if _, err := k8s.CoreV1().Secrets(cfg.Server.BrokerNamespace).Get(context.Background(), core.WorldWriteTokenRef(cfg, "no-such-world").Name, metav1.GetOptions{}); !apierrors.IsNotFound(err) {
...
func TestProvisionFileBackend(t *testing.T) {
	...
	cfg := &core.Config{
		Storage: core.StorageConfig{Backend: core.StorageBackendFile, Dir: filepath.Join(dir, "state")},
		Worlds: []core.WorldConfig{{
			Name:            "memory",
			TokensFile:      tokensFile,
			InternalAddress: "localhost:6309",
			DefaultToken:    core.TokenScope{Paths: []string{"/*"}},
		}},
	}
	store := newWorldWriteTokenStore(cfg, storage.NewFileSecretStore())
	...
	parsed, err := token.ParseBytes(brokertest.MustRead(t, tokensFile))
	if err != nil {
	...
	raw2, err := newWorldWriteTokenStore(cfg, storage.NewFileSecretStore()).Provision(context.Background(), "memory")
	if err != nil {
