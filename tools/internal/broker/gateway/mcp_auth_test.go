...
func TestGatewayAuthInvalidBearerReturns401WithErrorCode(t *testing.T) {
	...
	v := &brokertest.FakeVerifier{VerifyFn: func(string) (core.Claims, error) {
		return core.Claims{}, errors.New("forged signature")
	}}
...
func TestGatewayAuthRejectsUnverifiedEmail(t *testing.T) {
	...
	v := &brokertest.FakeVerifier{Claims: core.Claims{Subject: "google|alice", Email: "alice@example.com", EmailVerified: false}}
	ts := newTestMCPGateway(t, mcpTestConfig(), v)
...
func TestGatewayAuthRejectsEmptyEmailClaim(t *testing.T) {
	...
	v := &brokertest.FakeVerifier{Claims: core.Claims{Subject: "google|alice", Email: "   ", EmailVerified: true}}
	ts := newTestMCPGateway(t, mcpTestConfig(), v)
...
func TestGatewayAuthValidBearerStashesClaimsOnContext(t *testing.T) {
	...
	v := &brokertest.FakeVerifier{Claims: core.Claims{Subject: wantSubject, Email: "alice@example.com", EmailVerified: true}}

	g := newGatewayFixture(t, cfg, v, KnowledgeProfile()).gateway(&fakeDispatcher{})

	var seen *core.Claims
	var sawClaims bool
	recorder := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		seen, sawClaims = core.ClaimsFromCtx(r.Context())
...
func TestGatewayAuthEnforcesAllowDomains(t *testing.T) {
	for _, tt := range brokertest.AllowDomainCases() {
		t.Run(tt.Name, func(t *testing.T) {
			...
			g.gatewayAuth(brokertest.NoContent()).ServeHTTP(rec, brokertest.BearerRequest(raw))
