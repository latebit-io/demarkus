...
// refreshTokenBytes is 256 bits of entropy per opaque token; only its
// sha256 is stored, so a leaked Secret cannot be replayed.
...
func NewRefreshStore(cfg *core.Config, store core.SecretStore) *RefreshStore {
	return &RefreshStore{
		cfg:    cfg,
		store:  store,
		clock:  time.Now,
...
func (s *RefreshStore) Issue(ctx context.Context, claims *core.Claims, clientID string, ttl time.Duration) (string, error) {
	if ttl <= 0 {
	...
	err := s.store.Mutate(ctx, core.RefreshTokensRef(s.cfg), func(existing []byte) ([]byte, error) {
...
func (s *RefreshStore) Revoke(ctx context.Context, rawToken string) error {
	...
	keyHash := hashRefreshToken(rawToken)
	return s.store.Mutate(ctx, core.RefreshTokensRef(s.cfg), func(existing []byte) ([]byte, error) {
		if len(existing) == 0 {
			return existing, nil
		}
		store, err := decodeRefreshTokens(existing)
