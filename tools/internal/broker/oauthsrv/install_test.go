...
func installTestConfig() *core.Config {
...
// the install tests assert it never appears.
const testIssuancesNS = "broker-issuances"
...
func TestMeInstallHappyPathSingleWorld(t *testing.T) {
	...
	if _, err := k8s.CoreV1().Secrets(brokertest.BrokerNS).Get(context.Background(), testIssuancesNS, metav1.GetOptions{}); !apierrors.IsNotFound(err) {
...
func TestMeInstallNoBearerReturns401(t *testing.T) {
	srv, _ := newTestServer(t, installTestConfig(), &brokertest.FakeVerifier{Claims: brokertest.AliceClaims()}, fake.NewSimpleClientset())
...
func TestMeInstallBadBearerReturns401(t *testing.T) {
	verifier := &brokertest.FakeVerifier{VerifyFn: func(string) (core.Claims, error) {
		return core.Claims{}, errors.New("invalid token")
	}}
...
func TestMeInstallIncludesReadableWorldForNonWriter(t *testing.T) {
	cfg := installTestConfig()
	cfg.Worlds[0].Allow = core.AllowConfig{Domains: []string{"otherco.test"}}
	verifier := &brokertest.FakeVerifier{Claims: brokertest.AliceClaims()}
	srv, _ := newTestServer(t, cfg, verifier, fake.NewSimpleClientset())
...
func TestMeInstallUnverifiedEmailReturns403(t *testing.T) {
	...
	verifier := &brokertest.FakeVerifier{Claims: core.Claims{Email: "alice@example.com", EmailVerified: false, Subject: "g|alice"}}
	srv, _ := newTestServer(t, installTestConfig(), verifier, fake.NewSimpleClientset())
...
func TestMeInstallAcceptsBrokerSignedBearer(t *testing.T) {
	...
	primary := &brokertest.FakeVerifier{VerifyFn: func(string) (core.Claims, error) {
		return core.Claims{}, errors.New("primary verifier must not be invoked for broker-signed bearers")
	}}
	srv, _ := newTestServerWithSigner(t, cfg, primary, fake.NewSimpleClientset(), idTokenSigner)

	raw, err := idTokenSigner.Sign(&core.Claims{Subject: "u-alice", Email: "alice@example.com", EmailVerified: true}, cfg.Server.PublicURL, time.Hour, time.Now())
	if err != nil {
		t.Fatalf("Sign: %v", err)
	}
