...
func newTestAuthCodeStore(t *testing.T) (*authCodeStore, *brokertest.FakeClock) {
	t.Helper()
	c := brokertest.NewFakeClock(time.Date(2026, 5, 27, 12, 0, 0, 0, time.UTC))
...
func TestAuthCodeStoreBind(t *testing.T) {
	t.Run("issues a code, removes the pending entry, returns the request", func(t *testing.T) {
		...
		exchange := core.ExchangeResult{
			Claims:      core.Claims{Subject: "sub-1", Email: "alice@example.com", EmailVerified: true},
	...
	t.Run("unknown id returns not-found", func(t *testing.T) {
		store, _ := newTestAuthCodeStore(t)
		if _, _, err := store.Bind("nonexistent", &core.ExchangeResult{}); !errors.Is(err, errAuthCodePendingNotFound) {
	...
	t.Run("rebind on the same id is rejected", func(t *testing.T) {
		...
		if _, _, err := store.Bind(id, &core.ExchangeResult{RawIDToken: "first"}); err != nil {
			t.Fatalf("first Bind: %v", err)
		}
		_, _, err = store.Bind(id, &core.ExchangeResult{RawIDToken: "second"})
	...
	t.Run("expired pending is rejected", func(t *testing.T) {
		...
		clock.Advance(testPendingAuthCodeTTL + time.Second)
		if _, _, err := store.Bind(id, &core.ExchangeResult{}); !errors.Is(err, errAuthCodePendingNotFound) {
	...
	t.Run("codes are unique across many Binds", func(t *testing.T) {
		...
		for range 50 {
			...
			code, _, err := store.Bind(id, &core.ExchangeResult{})
			if err != nil {
				t.Fatalf("Bind: %v", err)
			}
			if seen[code] {
...
func TestAuthCodeStoreSweep(t *testing.T) {
	...
	codeExpired, _, err := store.Bind(idBindable, &core.ExchangeResult{})
	if err != nil {
		t.Fatalf("Bind: %v", err)
	}
...
func TestAuthCodeStoreConcurrentRedeemOneWinner(t *testing.T) {
	...
	code, _, err := store.Bind(id, &core.ExchangeResult{RawIDToken: "id-token"})
	if err != nil {
		t.Fatalf("Bind: %v", err)
	}
	verifier, _ := pkce("test-verifier-must-be-43-to-128-chars-long-1234")
