...
func NewDynamicClientStore(cfg *core.Config, store core.SecretStore) *DynamicClientStore {
...
func (s *DynamicClientStore) Register(ctx context.Context, clientID string, redirectURIs []string, name string) error {
	now := s.clock().UTC()
	return s.store.Mutate(ctx, core.DynamicClientsRef(s.cfg), func(existing []byte) ([]byte, error) {
...
func (s *DynamicClientStore) Lookup(ctx context.Context, clientID string) (dynamicClientRecord, bool, error) {
	...
	found := false
	err := s.store.Mutate(ctx, core.DynamicClientsRef(s.cfg), func(existing []byte) ([]byte, error) {
