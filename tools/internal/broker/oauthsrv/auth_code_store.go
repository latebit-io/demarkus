...
func (s *authCodeStore) Bind(id string, exchange *core.ExchangeResult) (string, PendingAuthCode, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	p, ok := s.pending[id]
	if !ok {
...
// not in the store).
func (s *authCodeStore) Redeem(code, clientID, redirectURI, codeVerifier string) (core.ExchangeResult, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	entry, ok := s.codes[code]
	if !ok {
		return core.ExchangeResult{}, errAuthCodeNotFound
	}
	...
	if subtle.ConstantTimeCompare([]byte(entry.ClientID), []byte(clientID)) != 1 {
		return core.ExchangeResult{}, errAuthCodeClientMismatch
	}
	if subtle.ConstantTimeCompare([]byte(entry.RedirectURI), []byte(redirectURI)) != 1 {
		return core.ExchangeResult{}, errAuthCodeRedirectMismatch
	}
	if !verifyPKCE(entry.CodeChallenge, entry.CodeChallengeMethod, codeVerifier) {
		return core.ExchangeResult{}, errAuthCodePKCEFailed
	}
	out := entry.Exchange
	delete(s.codes, code)
	return out, nil
