...
// testRefreshSecret names the refresh tokens Secret; distinct from the
// removed issuances Secret so a stray cross Secret read fails loudly.
...
func testRefreshConfig() *core.Config {
	c := brokertest.NewConfig()
...
func newRefreshStoreForTest(t *testing.T, k8s *fake.Clientset) *RefreshStore {
	t.Helper()
	s := NewRefreshStore(testRefreshConfig(), storage.NewK8sSecretStore(k8s))
...
func TestRefreshStoreIssueRejectsZeroTTL(t *testing.T) {
	k8s := fake.NewSimpleClientset()
	s := newRefreshStoreForTest(t, k8s)
	if _, err := s.Issue(context.Background(), &core.Claims{}, "", 0); err == nil {
		t.Fatalf("Issue with ttl=0 should error")
	}
	if _, err := s.Issue(context.Background(), &core.Claims{}, "", -time.Second); err == nil {
...
func TestRefreshStoreRefreshErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			s := newRefreshStoreForTest(t, k8s)
			ctx := context.Background()
			raw, err := s.Issue(ctx, &core.Claims{Subject: "u", Email: "a@b.com", EmailVerified: true}, "", time.Hour)
			if err != nil {
...
func TestRefreshStoreRevoke(t *testing.T) {
	t.Run("revoke valid then refresh fails", func(t *testing.T) {
		...
		s := newRefreshStoreForTest(t, k8s)
		ctx := context.Background()
		raw, err := s.Issue(ctx, &core.Claims{Email: "a@b.com", EmailVerified: true}, "", time.Hour)
		if err != nil {
			t.Fatalf("Issue: %v", err)
		}
	...
	t.Run("revoke unknown is no-op", func(t *testing.T) {
		...
		raw, err := s.Issue(context.Background(), &core.Claims{Email: "a@b.com", EmailVerified: true}, "", time.Hour)
		if err != nil {
			t.Fatalf("Issue: %v", err)
		}
...
func TestRefreshStoreSweep(t *testing.T) {
	t.Run("removes expired only", func(t *testing.T) {
		...
		shortRaw, err := s.Issue(ctx, &core.Claims{Email: "short@x.com", EmailVerified: true}, "", time.Hour)
		if err != nil {
			t.Fatalf("Issue short: %v", err)
		}
		longRaw, err := s.Issue(ctx, &core.Claims{Email: "long@x.com", EmailVerified: true}, "", 48*time.Hour)
		if err != nil {
	...
	t.Run("no-op when nothing expired", func(t *testing.T) {
		k8s := fake.NewSimpleClientset()
		s := newRefreshStoreForTest(t, k8s)
		_, err := s.Issue(context.Background(), &core.Claims{Email: "a@b.com", EmailVerified: true}, "", time.Hour)
		if err != nil {
			t.Fatalf("Issue: %v", err)
		}
		n, err := s.Sweep(context.Background())
...
func TestRefreshStoreSecretShape(t *testing.T) {
	...
	raw, err := s.Issue(ctx, &core.Claims{Subject: "u1", Email: "a@example.com", EmailVerified: true, Groups: []string{"g"}}, "", time.Hour)
	if err != nil {
		t.Fatalf("Issue: %v", err)
	}
	...
	body := sec.Data[core.RefreshTokensSecretKey]
	if len(body) == 0 {
		t.Fatalf("Secret data[%s] empty", core.RefreshTokensSecretKey)
	}
...
func TestRefreshStoreConcurrentRefresh(t *testing.T) {
	...
	for i := range n {
		raw, err := s.Issue(ctx, &core.Claims{Subject: fmt.Sprintf("u%d", i), Email: fmt.Sprintf("u%d@x.com", i), EmailVerified: true}, "", time.Hour)
		if err != nil {
...
func TestRefreshStoreIssueConflictRetries(t *testing.T) {
	...
	k8s.PrependReactor("update", "secrets", func(action k8stesting.Action) (bool, runtime.Object, error) {
		...
		if ua.GetNamespace() != brokertest.BrokerNS {
			return false, nil, nil
		}
	...
	seed := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{Name: testRefreshSecret, Namespace: brokertest.BrokerNS},
		Data:       map[string][]byte{},
	}
	if _, err := k8s.CoreV1().Secrets(brokertest.BrokerNS).Create(context.Background(), seed, metav1.CreateOptions{}); err != nil {
		t.Fatalf("seed: %v", err)
	}
...
func TestRefreshStoreCollisionRetry(t *testing.T) {
	...
	ctx := context.Background()
	if _, err := s.Issue(ctx, &core.Claims{Email: "a@b.com", EmailVerified: true}, "", time.Hour); err != nil {
...
func TestRefreshStoreFileBackend(t *testing.T) {
	...
	cfg.Storage = core.StorageConfig{Backend: core.StorageBackendFile, Dir: t.TempDir()}
	s := NewRefreshStore(cfg, storage.NewFileSecretStore())
	s.clock = func() time.Time { return time.Date(2026, 5, 15, 12, 0, 0, 0, time.UTC) }
	ctx := context.Background()

	claims := core.Claims{Subject: "user-1", Email: "alice@example.com", EmailVerified: true}
	...
	s2 := NewRefreshStore(cfg, storage.NewFileSecretStore())
	s2.clock = s.clock
