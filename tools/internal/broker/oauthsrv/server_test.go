...
func TestNewServerPanicsOnDiscoveryWithoutSigner(t *testing.T) {
	...
	deps := testServerDeps(t, cfg, &brokertest.FakeVerifier{}, fake.NewSimpleClientset())
	deps.Discovery = d
	NewServer(cfg, deps)
}
...
func TestWellKnownDiscoveryRouteRegistered(t *testing.T) {
	...
	// otherwise.
	deps := testServerDeps(t, cfg, &brokertest.FakeVerifier{}, fake.NewSimpleClientset()).signed(cfg, brokertest.NewTestIDTokenSigner(t))
	deps.Discovery = d
...
func TestAuthCallbackSuccess(t *testing.T) {
	...
	// The callback lists a world only with a PublicURL, as /me/install
	// does; the fixture's default is empty.
...
func TestAuthCallbackStateMismatch(t *testing.T) {
	verifier := &brokertest.FakeVerifier{Claims: core.Claims{Email: "alice@example.com", EmailVerified: true}}
	srv, _ := newTestServer(t, brokertest.NewConfig(), verifier, fake.NewSimpleClientset())
...
func TestAuthCallbackExchangeFails(t *testing.T) {
	verifier := &brokertest.FakeVerifier{ExchErr: errors.New("idp said no")}
	srv, _ := newTestServer(t, brokertest.NewConfig(), verifier, fake.NewSimpleClientset())
	client, nonce := loginAndExtract(t, srv)
...
func TestAuthCallbackUnauthorizedDomain(t *testing.T) {
	verifier := &brokertest.FakeVerifier{Claims: core.Claims{Email: "mallory@evil.example", EmailVerified: true, HD: "evil.example"}}
	cfg := brokertest.NewConfig()
