...
// Memory-broker prompts: the soul workflows the demarkus-memory plugin
...
func (g *mcpGateway) registerMemoryPrompts() {
	...
	g.mcpServer.AddPrompt(mcp.NewPrompt("soul-journal",
		mcp.WithPromptDescription("Record a dated journal entry in your soul: append what happened this session to today's journal document."),
...
func soulContextPrompt(_ context.Context, req mcp.GetPromptRequest) (*mcp.GetPromptResult, error) { //nolint:gocritic // signature required by mcp-go
	...
	text := fmt.Sprintf(`Restore working context from this soul.%s
...
5. Report a short brief: what was in progress, recent decisions, and anything flagged as next. Cite each claim as mark://<w>/<path>. If the soul is freshly seeded and has no history, say so plainly.`, focusLine)
	return mcp.NewGetPromptResult("Soul context", []mcp.PromptMessage{
		mcp.NewPromptMessage(mcp.RoleUser, mcp.NewTextContent(text)),
	}), nil
}

func soulJournalPrompt(_ context.Context, req mcp.GetPromptRequest) (*mcp.GetPromptResult, error) { //nolint:gocritic // signature required by mcp-go
	...
	text := fmt.Sprintf(`Record a dated journal entry in this soul.
...
5. If the entry records a decision or a lesson worth finding later, also update the matching document (/adr/, /debugging.md, /patterns.md) and keep mark://<w>/index.md current.`, entryLine)
	return mcp.NewGetPromptResult("Soul journal", []mcp.PromptMessage{
		mcp.NewPromptMessage(mcp.RoleUser, mcp.NewTextContent(text)),
	}), nil
}
