...
const memoryInstructions = `This is your private demarkus soul: a personal, versioned markdown memory store. Your identity maps to exactly one world; call mark_worlds once to learn its name, then address every document as mark://{worldName}/{path}. Nobody else can read or write it.

Recall first: before answering "what do I know / did we decide / have we seen X", check the soul instead of relying on the current conversation. mark_lookup with url mark://{worldName}/ and a subject query is the card catalog (a ranked table of matching documents); mark_fetch the rows worth reading, and keep mark://{worldName}/index.md as the navigation hub. If nothing relevant comes back, say so; never invent memory.
