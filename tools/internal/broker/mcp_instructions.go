...
Reading: call mark_worlds once to discover readable worlds; surface failures and stop. Respect explicit world scope with mark_lookup; otherwise mark_lookup_all is the system-wide card catalog. ` + mcpfmt.SectionFirst + ` Use descriptive subjects, catalog for names/tags, match=body for section text or catalog misses. Fetch the best section directly; mark_explore only when orientation is needed. A short unanchored document can be read whole. Optional budget=1500 is for focused body queries, not broad name lookup. Use a world's index.md as the untagged-content backstop. Surface fetch/explore failures and disclose partial lookups; only a successful non-partial empty lookup means nothing found.
