...
Recall first: before answering "what do I know / did we decide / have we seen X", check the memory instead of relying on the current conversation. mark_lookup with url mark://{worldName}/ and a subject query is the card catalog (a ranked table of matching documents), match=body when the subject is in section text rather than tags; mark_fetch rows at their #anchor, a whole document only when the outline shows the answer spans sections, and keep mark://{worldName}/index.md as the navigation hub. If nothing relevant comes back, say so; never invent memory.
