...
# Derived from SYSTEMD_DIR (must follow the testmode override above).
WORLD_DROPIN_DIR="${SYSTEMD_DIR}/demarkus.service.d"
...
#     certs on SIGHUP when it started WITH a cert, i.e. prodMode);
#   - renewal: same path, content changed -> SIGHUP reloads in place.
# A timer runs it every 12h so renewals land without inotify on Caddy's
# not-yet-existing cert paths.
write_certsync() {
  local host="$1"
...
if ! cmp -s "\$src_crt" "\$dst_crt" || ! cmp -s "\$src_key" "\$dst_key"; then
  install -m 640 -o root -g "\$WORLD_GROUP" "\$src_crt" "\$dst_crt"
  install -m 640 -o root -g "\$WORLD_GROUP" "\$src_key" "\$dst_key"
  changed=1
fi

if [ ! -f "\$DROPIN" ]; then
  # First issuance: point the world at the real cert and restart so it starts
  # in prodMode (SIGHUP cert reload is a no-op otherwise).
  mkdir -p "\$DROPIN_DIR"
  cat > "\$DROPIN" <<UNIT
[Service]
