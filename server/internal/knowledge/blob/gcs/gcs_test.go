...
func TestProviderErrorClassification(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "CRC create rejection", ctx: context.Background(), op: "create", cause: errors.New("storage: object checksum mismatch"), want: []error{blob.ErrIntegrity, blob.ErrAmbiguous}},
		{name: "CRC replace rejection", ctx: context.Background(), op: "replace", cause: errors.New("storage: object checksum mismatch"), want: []error{blob.ErrIntegrity, blob.ErrAmbiguous}},
