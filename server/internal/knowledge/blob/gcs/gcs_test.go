...
func TestProviderErrorClassification(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "CRC read message", ctx: context.Background(), op: "get", cause: errors.New("storage: bad CRC on read"), want: []error{blob.ErrUnavailable}},
		{name: "generic checksum read message", ctx: context.Background(), op: "get", cause: errors.New("storage: object checksum mismatch"), want: []error{blob.ErrUnavailable}},
		{name: "untyped checksum create message", ctx: context.Background(), op: "create", cause: errors.New("storage: object checksum mismatch"), want: []error{blob.ErrUnavailable, blob.ErrAmbiguous}},
		{name: "untyped checksum replace message", ctx: context.Background(), op: "replace", cause: errors.New("storage: object checksum mismatch"), want: []error{blob.ErrUnavailable, blob.ErrAmbiguous}},
		{name: "checksum bad request remains permanent", ctx: context.Background(), op: "create", cause: &googleapi.Error{Code: http.StatusBadRequest, Message: "CRC32C does not match"}},
