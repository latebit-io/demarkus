...
func (s *Store) ExportDocs(ctx context.Context, fn func(reqPath string, document storefmt.StoredDocument) error) error {
	...
	return filepath.WalkDir(root, func(path string, d fs.DirEntry, err error) error {
		...
		return fn(reqPath, storefmt.StoredDocument{Versions: versions, Archived: archived})
	})
}
...
func (s *Store) exportArchived(reqPath string, tipStored []byte) (bool, error) {
	...
	return effectiveArchived(loc.docDir, tipStored)
}

// exportVersions reads a document's version files oldest-first.
func (s *Store) exportVersions(reqPath string) ([]storefmt.StoredVersion, error) {
	infos, err := s.Versions(reqPath)
	if err != nil {
		return nil, err
	}
	versions := make([]storefmt.StoredVersion, 0, len(infos))
	...
	for i := len(infos) - 1; i >= 0; i-- {
		...
		versions = append(versions, storefmt.StoredVersion{Version: infos[i].Version, Stored: stored, Modified: infos[i].Modified})
	}
	return versions, nil
}
...
func (s *Store) ImportDoc(ctx context.Context, reqPath string, document storefmt.StoredDocument) error {
	if err := storefmt.ValidateImport(reqPath, document); err != nil {
		return err
	}
	if err := ctx.Err(); err != nil {
		return err
...
func (s *Store) importVersionFiles(ctx context.Context, reqPath string, versions []storefmt.StoredVersion) error {
